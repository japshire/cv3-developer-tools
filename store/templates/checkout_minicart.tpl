...
<div>
 ...
 {foreach from=$cart key=key item=prod}
  ...
  {if $prod.qty > 0 && $prod.build_prod_id == ''}
   ...
   <div class='mini-product clearfix'>
    ...
    <div style="margin-left:65px; margin-bottom:5px; padding-left:10px">
 ...
 {*include_php file="services/ditalia/shipDropdown2.php"*}
...
*}
   ...
   {/if}
     ...
     {/if}
     <h2>Sub-Total: {$currency_type}{$grandtotal|commify}</h2>
{*   <h2>Sub-Total: 
{if $settings.hide_viewcart_discounts == 'n' && $totals.price > $totals.discount_price}
