 ...
 {elseif $view == category || $view == prod_detail_list}
  ...
  {foreach from=$p.attributes.attributes item=attribute key=key}
          ...
          {foreach from=$attribute2.dependents item=attribute3 key=key3}
            {if $p.attributes.num > 3}
              {foreach from=$attribute3.dependents item=attribute4 key=key4}
                {if $attribute4.status != 'discontinued' && $attribute4.price > $maxPrice}
                  {assign var="maxPrice" value=$attribute4.price}
                {/if}
                {if $attribute4.status != 'discontinued' && $attribute4.price != '' && isset($attribute4.price) && $attribute4.price < $minPrice}
                  {assign var="minPrice" value=$attribute4.price}
                {/if}
                {if $attribute4.status != 'discontinued' && $attribute4.standard_price > $smaxPrice}
                  {assign var="smaxPrice" value=$attribute4.standard_price}
                {/if}
                {if $attribute4.status != 'discontinued' && $attribute4.standard_price != '' && isset($attribute4.price) && $attribute4.standard_price < $sminPrice}
                  {assign var="sminPrice" value=$attribute4.standard_price}
                {/if}
                {if $attribute4.status != 'discontinued' && !isset($attribute4.price)}
                  {assign var="factorinparent" value=1}
                {/if}
              {/foreach}
            {else} {* just 3 attys *}
              {if $attribute3.status != 'discontinued' && $attribute3.price > $maxPrice}
                {assign var="maxPrice" value=$attribute3.price}
              {/if}
              {if $attribute3.status != 'discontinued' && $attribute3.price != '' && isset($attribute3.price) && $attribute3.price < $minPrice}
                {assign var="minPrice" value=$attribute3.price}
              {/if}
              {if $attribute3.status != 'discontinued' && $attribute3.standard_price > $smaxPrice}
                {assign var="smaxPrice" value=$attribute3.standard_price}
              {/if}
              {if $attribute3.status != 'discontinued' && $attribute3.standard_price != '' && isset($attribute3.price) && $attribute3.standard_price < $sminPrice}
                {assign var="sminPrice" value=$attribute3.standard_price}
              {/if}
              {if $attribute3.status != 'discontinued' && !isset($attribute3.price)}
                {assign var="factorinparent" value=1}
              {/if}
            {/if}
          {/foreach}
        {else} {* just 2 attys *}
          {if $attribute2.status != 'discontinued' && $attribute2.price > $maxPrice}
            {assign var="maxPrice" value=$attribute2.price}
          {/if}
          {if $attribute2.status != 'discontinued' && $attribute2.price != '' && isset($attribute2.price) && $attribute2.price < $minPrice}
            {assign var="minPrice" value=$attribute2.price}
          {/if}
          {if $attribute2.status != 'discontinued' && $attribute2.standard_price > $smaxPrice}
            {assign var="smaxPrice" value=$attribute2.standard_price}
          {/if}
          {if $attribute2.status != 'discontinued' && $attribute2.standard_price != '' && isset($attribute2.price) && $attribute2.standard_price < $sminPrice}
            {assign var="sminPrice" value=$attribute2.standard_price}
          {/if}
          {if $attribute2.status != 'discontinued' && !isset($attribute2.price)}
            {assign var="factorinparent" value=1}
          {/if}
        {/if}
      {/foreach}
    {else} {* just 1 atty *}
      ...
      {/if}
      {if $attribute.status != 'discontinued' && $attribute.price != '' && isset($attribute.price) && $attribute.price < $minPrice}
        {assign var="minPrice" value=$attribute.price}
      {/if}
      {if $attribute.status != 'discontinued' && $attribute.standard_price > $smaxPrice}
        {assign var="smaxPrice" value=$attribute.standard_price}
      {/if}
      {if $attribute.status != 'discontinued' && $attribute.standard_price != '' && isset($attribute.price) && $attribute.standard_price < $sminPrice}
        {assign var="sminPrice" value=$attribute.standard_price}
      {/if}
      {if $attribute.status != 'discontinued' && !isset($attribute.price)}
       {assign var="factorinparent" value=1}
      {/if}
    {/if}
  {/foreach}
  ...
  {elseif $p.has_attributes == 'y' && $attPrices != 'y'}
   {foreach from=$attributes.attributes item=attribute key=key}
    {foreach from=$attribute.dependents item=dependent key=dkey}
     ...
     {/if}
     {if $dependent.status != 'discontinued' && ($dependent.price < $minPrice || $minPrice == '0')}
   ...
   {else}
     ...
     {else}
      ...
      {else}
        <input type="text" class="dynamic_qty{if $p.special == 'y' && $p.is_donation != 'y'} sale-price{elseif $p.is_donation == 'y'} donation{/if}" id="price_{$prod_id}" {if $p.price > 0}value="{$currency_type}{if $p.min_qty > 1}{math equation="x*y" x=$p.price y=$p.min_qty assign="tmp_price"}{$tmp_price|commify}{else}{$p.price|commify}{/if}"{else}value="View for pricing"{/if} readonly="readonly" onfocus="this.blur()" />
      {/if}
     {/if}
   {/if}
