{* ------[ This beginning part is just variable definitions that could possibly be moved to the back-end code ]------ *}
...
{if $pops}
 {include file="quickshop_popup.tpl"}
{else}
 {if !$child}
...
{* kg - product names and cat header on cat pages *}
          <div class="content">
           {if $view == category}
            {if !$child}<a href='{$smarty.capture.product_url}'>{$product.prod_name}</a><br>{if $product.desc_header != ''}{$product.desc_header}{/if}
           ...
           {else}
            ...
            {if $product.desc_header != ''}<h2 class="subtext">{$product.desc_header}</h2>{/if}
           ...
           {if $product.children|@count == ''}
            ...
            {/if}
            {if $product_status == 'Out of Stock'}<div class='status'>{$product_status}</div>{/if}
           {/if}
            ...
            {if $product.is_parent != 'y' && $product.has_attributes != 'y' && $product.gift_set != 'y' && $product.text_field != 'y' && ($product.form_id == 0 || $product.form_id == '') && ($product.gift_certificate != 'y' || ($product.gift_certificate_value > '0' && $product.gift_certificate_send != 'y'))}
             ...
             {if $cid == 'rp'}
               {if $member.points_available >= $product.rewards_points}
                 ...
                 <input type="submit" name="addtocart_submit" class="button small style1" value="Add to Cart" onclick="javascript: return checkMinQty('qty_{$prod_id}',{$product.min_qty},'');" />
               {/if}
             {else}
             <input type="submit" name="addtocart_submit" class="button small style1" value="Add to Cart" onclick="javascript: return checkMinQty('qty_{$prod_id}',{$product.min_qty},'');" />
             {/if}
          ...
          {/if}
           ...
           {if $product.has_attributes == 'y' && $allow_purchase}<div class="attributes">{include file="attributes.tpl"}</div>{/if}
    ...
    {$minPoints}
           {if $member.points_available >= $product.rewards_points}
             <input type="hidden" name="reward_amount" id="reward_amount_{$prod_id}" value="{$product.rewards_points}" />
             <div class="btn-add"><input type="submit" name="addtocart_submit" class="button small style1" value="Add to Cart" onclick="javascript: return checkMinQty('qty_{$prod_id}',{$product.min_qty},'');"/></div>
           {/if}
         {else}
             <div class="btn-add"><input type="submit" name="addtocart_submit" class="button small style1" value="Add to Cart" onclick="javascript: return checkMinQty('qty_{$prod_id}',{$product.min_qty},'');"/></div>
         {/if}
         {if $memberinfo.id}<div class="btn-wishlist"><input type="submit" name="wishlist_add" class="button small style2" value="Add to Wishlist" /></div>{/if}
         {*if $memberinfo.id}<div class="btn-reorder"><input type="submit" name="reorder_add" class="button small style2" value="Easy Reorder" /></div>{/if*}

        {/if}
        {if $notify && $settings.inventory_control == 'y' && $settings.product_notify == 'y' && $product.notify == 'y'}<div><a href="{$notify_url}">Notify me when this item is in stock</a></div>{/if}
       {/if}
...
**********}
 ...
 <div class='image iefix pull-right' style="margin-bottom:20px">
  {if $product.content_only == 'y'}
   {*eval var=$product.prod_description*}
  {*else*}   
<h1>{$product.prod_name}
{if $product.cust_14 != ''}<div class="subtext">{$product.cust_14}</div>{/if}
</h1><br><br>
   {if $image}
    <div id="img_large" style="float:right;margin:0 0 20px 20px">
...
{*** END JS FOR IMAGE SWAP ***}
 ...
 <div class='info iefix pull-left'>
  ...
  <div class="content">
   {if $view == category}
            {if !$child}<a href='{$smarty.capture.product_url}'>{$product.prod_name}{if $product.desc_header != ''}, {$product.desc_header}{/if}</a>
    {else}<h2>{$product.prod_name}</h2>
    {/if}
   {elseif ($view == product || view == 'product_builder') && $isAdditional == y}
    <h2>{$product.prod_name}</h2>
   {else}
    <h1>{$product.prod_name}
    {if $product.desc_header != ''}<div class="subtext">{$product.desc_header}</div>{/if}</h1>
