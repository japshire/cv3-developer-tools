    ...
    {foreach from=$product.children item=child}
        ...
        {if ($child.stock_status == 'In Stock' || ($child.stock_status == 'Backordered' && $child.backordered_date != '')) && $product.outseason != 'y' && $child.outseason != 'y'}
            ...
            {if $settings.recurring_orders_by_product == 'y' && $product.recurring_eligible == 'y'}
                    <br/>
                    <div id="recurring_eligible_{$child.prod_id}"><legend>Eligible for recurring order:</legend><br/>
                      <div class="controlset">
                        <input type="radio" id="recurring_no_{$child.prod_id}" name="recurring_yesno{*if $settings.additional_prod_display_type == '2'*}{$child.prod_id}{*/if*}" value="n" checked/> <label for="recurring_no_{$child.prod_id}">Only order once</label><br/>
                        <input type="radio" id="recurring_yes_{$child.prod_id}" name="recurring_yesno{*if $settings.additional_prod_display_type == '2'*}{$child.prod_id}{*/if*}" value="y" /><label for="recurring_yes_{$child.prod_id}"> Order Every
                          <input type="text" id="recurring_interval_num_{$child.prod_id}" name="recurring_interval_num{*if $settings.additional_prod_display_type == '2'*}{$child.prod_id}{*/if*}" value="0"/>
                          <select name="recurring_interval{*if $settings.additional_prod_display_type == '2'*}{$child.prod_id}{*/if*}">
