...
{foreach from=$featured item=prod}
 {cycle values="f_left,f_center,f_center2,f_right" assign="f_which"}
 {if $f_which == "f_left"}
 ...
 {$f_begin}
  ...
  <table width="150" cellspacing="0" cellpadding="0" align="center">
    <tr>
      <td align="center">
        <a href="/product/{if $prod.url_name != ''}{$prod.url_name}{else}{$prod.prod_id}{/if}/{if $prod.cid_name != ''}{$prod.cid_name}{else}{$prod.cid}{/if}"><img src="{if $prod.web_image != ''}{$prod.web_image|img_prefix}{else}{$img_prefix}/images/nopicture_tn.jpg{/if}" alt="{$prod.prod_name|replace:"&":"&amp;"|strip_quotes|strip_tags:false}" /></a>
        <div class="prod_links">
          <a href="/product/{if $prod.url_name != ''}{$prod.url_name}{else}{$prod.prod_id}{/if}/{if $prod.cid_name != ''}{$prod.cid_name}{else}{$prod.cid}{/if}">{$prod.prod_name|replace:"&":"&amp;"}</a>
        </div>
{*        <div class="productprice">
          {if $prod.price != '' && $prod.price != '0.00' && $prod.price != 'Various Options Available' && $prod.build_product != 'y'}
            {if $prod.special == 'y'}
