...
<head>
    <title>{if $view == 'create_review'}Create Review {elseif $view == 'view_reviews'}View Reviews {/if}{$title|replace:"& ":"&amp; "|strip_quotes|strip_tags:false}</title>
    ...
    <link rel="stylesheet" type="text/css" href="{#stylesheet#|cdn_styles}">
       ...
       <link rel="stylesheet" type="text/css" href="https://cloud.typography.com/7239976/7112172/css/fonts.css" />
            ...
            {* FACEBOOK/PINTEREST RICH PINS TAGS - uncomment to use *}
            {if $view == 'product'}
            ...
            {elseif  $product.is_parent != 'y' && $product.additional_products != ''}
                ...
                {/foreach}
            {/if} 
            {*<meta property="fb:app_id" content="THIS MUST BE CREATED ON FACEBOOK PRIOR TO USE"/>*}
        {/if}

{* CANONICAL LINKS *}
{if $view == 'category' || $view == 'prod_detail_list'}
  <link rel="canonical" href="{$protocol}://{$smarty.server.HTTP_HOST}/{$view}/{if $categories.$cid.url_name != ''}{$categories.$cid.url_name}{else}{$cid}{/if}" />
...
{/if}
        ...
        {* THIS LOADS THE JQUERY LIBRARY VIA CDN - latest version as of 06/12/2015 *}
