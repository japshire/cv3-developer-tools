...
{else}
 ...
 {else}
  ...
  {else} 
   ...
   {if $image}
    <div id="img_large">
     {if $product.full_image != ''}<a href="{$product.full_image}" rel="lightbox" id="prod_link_{$prod_id}">{/if}<img src="{if $product.large_image == ''}{$img_prefix}/images/nopicture_tn.jpg{else}{$product.large_image|img_prefix}{/if}" border="0" alt="{$product.prod_name|replace:"&":"&amp;"|strip_quotes|strip_tags:false}" id="prod_image_{$prod_id}" />{if $product.full_image != ''}</a>{/if}
    </div>
...
{****
    <div id="social_btns">
     ...
     <script type="text/javascript" src="http://s7.addthis.com/js/250/addthis_widget.js#pubid=ra-4ec550620a13976e"></script>
     <!-- AddThis Button END -->
    </div>
**}
...
<h1>{$product.prod_name}
   ...
   {if $view == 'product' || view == 'product_builder'}
    ...
    {include file="product_kit.tpl"}
	    ...
	    {** Commenting bellow section to hide the Recipe text **}
  ...
  <div class="form clearfix {if $product.children|@count == ''}borbtm{/if}">
